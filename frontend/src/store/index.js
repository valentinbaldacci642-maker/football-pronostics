...
export const useHistoryStore = create(
  persist(
    (set, get) => ({
      ...
      // One-shot manual import: replace the entire historique by a hardcoded
      // list of bets read off the bookmaker's app. Both settled and pending
      // bets are inserted, so 'Paris en cours' and 'Historique pronos' both
      // populate cleanly from one click.
      seedUnibetBets: () => {
        // Each entry = one match. bets[] are the wagers placed on it.
        // Settled bets carry `result` ('win' | 'loss'); pending bets leave
        // `result` undefined so they show in 'Paris en cours'.
        // Cashouts are represented as `result: 'win'` with odd = return / mise
        // so totalReturn matches what the bookmaker actually paid.
        const SEED = [
          // ── SETTLED ─────────────────────────────────────────────────────

          // Wolfsburg vs Bayern Munich — Bundesliga — 09/05 18:30
          ...
          // Manchester City vs Brentford — Premier League — 09/05 18:30
          ...
          // Hoffenheim vs Werder Bremen — Bundesliga — 09/05 15:30
          ...
          // RB Leipzig vs FC St. Pauli — Bundesliga — 09/05 15:30
          ...
          // Stuttgart vs Leverkusen — Bundesliga — 09/05 15:30
          ...
          // Bayern Munich vs Paris SG — UCL — 06/05 21:00
          ...
          // Al Ahli SC vs Al Fateh — Saudi Pro — 06/05 20:00
          { home: 'Al Ahli SC', away: 'Al Fateh', league: 'Saudi Pro League',
            ...
            bets: [
              ...
              // Cashout 0.20€ sur 0.22€ mise → 'win' avec odd = 0.20/0.22
              { market: '1X2 (cashout)', selection: 'X', odd: 0.20 / 0.22, mise: 0.22, result: 'win' },
            ] },

          // Lecce vs Juventus Turin — Serie A — 09/05 20:45
          ...
          // Chicago Fire vs NY Red Bulls — MLS — 09/05 20:30
          ...
          // Reims vs Pau — Ligue 2 — 09/05 20:00
          ...
          // St Etienne vs Amiens — Ligue 2 — 09/05 20:00
          ...
          // Gwangju FC vs Gangwon FC — K League 1 — 09/05 09:30
          ...
          // Lens vs Nantes — Ligue 1 — 08/05 20:45
          ...
          // Dortmund vs Eintracht Frankfurt — Bundesliga — 08/05 20:30
          ...
          // Paderborn vs Karlsruhe — Bundesliga 2 — 08/05 18:30
          ...
          // FK RFS vs FK Liepaja — Virsliga LV — 08/05 18:00
          ...
          // CA Platense vs CA Penarol — Libertadores — 08/05 00:00 UTC
          { home: 'CA Platense', away: 'CA Penarol', league: 'CONMEBOL Libertadores',
            ...
            bets: [
              ...
              // Cashout 0.49€ sur 0.54€ mise → 'win' avec odd = 0.49/0.54
              { market: 'O/U 2.5 (cashout)', selection: 'Plus 2.5', odd: 0.49 / 0.54, mise: 0.54, result: 'win' },
            ] },

          // Hellas Verona vs Como — Serie A — 10/05 12:30 — settled (loss)
          ...
          // GOAhead Eagles vs PSV — Eredivisie — 10/05 16:45
          { home: 'GOAhead Eagles', away: 'PSV Eindhoven', league: 'Eredivisie',
            matchDate: '2026-05-10T16:45:00+02:00',
            bets: [
              { market: 'O/U 2.5', selection: 'Under 2.5', odd: 3.00, mise: 0.10 },
            ] },

          // Barcelona vs Real Madrid — LaLiga — 10/05 21:00
          { home: 'FC Barcelona', away: 'Real Madrid', league: 'LaLiga',
            matchDate: '2026-05-10T21:00:00+02:00',
            bets: [
              { market: 'O/U 2.5', selection: 'Under 2.5', odd: 3.40, mise: 0.16 },
            ] },

          // Twente vs Sparta Rotterdam — Eredivisie — 10/05 16:45
          { home: 'Twente', away: 'Sparta Rotterdam', league: 'Eredivisie',
            matchDate: '2026-05-10T16:45:00+02:00',
            bets: [
              { market: '1X2', selection: 'X', odd: 4.95, mise: 0.14 },
            ] },
        ];
        ...
        // Wipe everything else: this seed represents the canonical state
        // from the bookmaker, so we don't merge with stale local entries.
        set({ entries: seeded });
      },
